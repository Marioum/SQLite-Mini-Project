...
public class DataBase extends SQLiteOpenHelper {
    ...
    public boolean update_OS_data (Fichier_info info) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(OS_DATA_COLUMN_LOGO, info.getLogo());
        contentValues.put(OS_DATA_COLUMN_DESCR, info.getDescription());
        ...
        long result= db.update(OS_DATA_TABLE_NAME , contentValues, OS_DATA_COLUMN_OS+" = ? ", new String[] {info.getOps()} );
        if (result==-1)
            return false;
        return true;
    }

    public ArrayList<String> getAll_OS_name() {
        ArrayList<String> array_list = new ArrayList<String>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor res =  db.rawQuery( "select * from "+OS_DATA_TABLE_NAME, null );
        res.moveToFirst();
        while(res.isAfterLast() == false){
            array_list.add(res.getString(res.getColumnIndex(OS_DATA_COLUMN_OS)));
            res.moveToNext();
        }
        return array_list;
    }

    // récuperer un seul os-data
